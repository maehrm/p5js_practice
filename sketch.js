...
function draw() {
    ...
    x = sin(radians(angle1)) * r1; // 円周上（大）のX座標の位置
    y = cos(radians(angle1)) * r1; // 円周上（大）のY座標の位置
    push();                        // 座標を円周上（大）の(X,Y)座標にするため一時的に保存
    translate(x, y);            
    for (let i = 0; i < 8; i++) {
        x = sin(radians(angle2 - (i * 45))) * r2; // 円周上のX座標の位置
        y = cos(radians(angle2 - (i * 45))) * r2; // 円周上のY座標の位置
        fill(35 * i);
